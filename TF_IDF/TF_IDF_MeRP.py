...
with open('D:/postgraduate0/Medical-Report-Generation-TriNet/Medical-Report-Generation-TriNet-main/data/CN/Mammary/Mammary_annotation.json', 'r', encoding='utf-8-sig') as input:
    DATA = json.load(input)

with open('D:/postgraduate0/Medical-Report-Generation-TriNet/Medical-Report-Generation-TriNet-main/data/CN/Mammary/vocab_TF-IDF.json', 'r', encoding='utf-8-sig') as input:  # using the vocabulary of TF-IDF construction which can be different from the report generation vocabulary
...
for key in DATA["train"]:
    b = list(jieba.cut(key['finding'].replace('。', ''))) # 分词
    k.append(key['uid']) # k存储的是report的idx
    # 检查句子中的每个词是否在词典中，没有更换为"<unk>"
    for i in range(len(b)): 
        if b[i] not in vocab:
            b[i] = "<unk>"
    b = " ".join(b) # 重新组装成一个string，词与词用' '隔开
    docs.append(b) # docs存放的是report，每个report就是一个元素，每个report是一个string，没有句号，句子与句子之间用空格隔开

docs_words = [d.replace(",", "").split(" ") for d in docs] # 分词，每个元素是一个list，list中装着该report的词汇
vocab = set(itertools.chain(*docs_words)) # itertools.chain() 去除iterable中的内嵌iterable，即将docs_words的每个元素（list）拆开，得到一个由所有report词汇组成的list
# set用于去掉重复元素
...
def get_tf(method="log"):
    ...
    for i, d in enumerate(docs_words):
        counter = Counter(d) # 一个report中词汇出现的频次
        for v in counter.keys():
            _tf[v2i[v], i] = counter[v] / counter.most_common(1)[0][1]
            # Counter().most_common(n)：返回出现频率最高的前n个词汇组成的list，list的元素是一个tuple，tuple由词汇(str)和出现次数组成
...
with open('D:/postgraduate0/Medical-Report-Generation-TriNet/Medical-Report-Generation-TriNet-main/data/CN/Mammary/TF_IDF_Report.json', 'w') as fp:
