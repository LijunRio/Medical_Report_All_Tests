...
class DebuggerBase:
    ...
    def train(self):
        for epoch_id in range(self.start_epoch, self.args.epochs):
            ...
            train_stop_loss, train_word_loss, train_loss = self._epoch_train()
            # val_tag_loss, val_word_loss, val_loss = self._epoch_val()
            val_stop_loss, val_word_loss, val_loss = self._epoch_val()
            
            '''
            writer = SummaryWriter()
            if epoch_id % 5 == 0:
                writer.add_scalar('train loss', train_loss, epoch_id)
                writer.add_scalar('val loss', val_loss, epoch_id)
            '''
...
class LSTMDebugger(DebuggerBase):
    ...
    def _epoch_train(self):
        ...
        # for i, (images, _, label, captions, prob) in enumerate(tqdm(self.train_data_loader)):
        for i, (images, _, captions, prob) in enumerate(tqdm(self.train_data_loader)):
            ...
            # semantic_features = self.mlc.forward(avg_features)
            ...
            for sentence_index in range(captions.shape[1]):
                ctx, _ = self.co_attention.forward(avg_features,
                                                      prev_hidden_states)
    ...
    def _epoch_val(self):
        ...
        # for i, (images, _, label, captions, prob) in enumerate(tqdm(self.val_data_loader)):
        for i, (images, _, captions, prob) in enumerate(tqdm(self.val_data_loader)):
            # batch_tag_loss, batch_stop_loss, batch_word_loss, batch_loss = 0, 0, 0, 0
            batch_stop_loss, batch_word_loss, batch_loss = 0, 0, 0
            ...
            for sentence_index in range(captions.shape[1]):
                ...
                ctx, v_att, a_att = self.co_attention.forward(avg_features,
                                                              ...
                                                              prev_hidden_states)
                                                              '''
                ctx, v_att= self.co_attention.forward(avg_features,
                                                              # semantic_features,
                                                              prev_hidden_states)
        ...
        # return tag_loss, word_loss, loss
        return stop_loss, word_loss, loss
...
if __name__ == '__main__':
    ...
    # Path Argument
    parser.add_argument('--vocab_path', type=str, default='./data/new_data/CN/Thyroid/vocab.pkl',
                        help='the path for vocabulary object')
    parser.add_argument('--image_dir', type=str, default='/home/mist/Ultrasonic_datasets/Throid_dataset/Thyroid_images',
                        help='the path for images')
    parser.add_argument('--caption_json', type=str, default='./data/new_data/CN/Thyroid/captions.json',
                        help='path for captions')
    parser.add_argument('--train_file_list', type=str, default='./data/new_data/CN/Thyroid/train_data.txt',
                        help='the train array')
    parser.add_argument('--val_file_list', type=str, default='./data/new_data/CN/Thyroid/test_data.txt',
                        help='the val array')
    # transforms argument
