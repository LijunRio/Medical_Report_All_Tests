...
class DebuggerBase:
    ...
    def _init_semantic_embedding(self):
        model = SemanticEmbedding(report_dim=self.args.report_dim,
    ...
    def train(self):
        ...
        for epoch_id in range(self.start_epoch, self.args.epochs):
            ...
            val_met = self._epoch_val()
            test_met = self._epoch_test()
            b1 = test_met['BLEU_1']
            b2 = test_met['BLEU_2']
            b3 = test_met['BLEU_3']
            b4 = test_met['BLEU_4']
...
class LSTMDebugger(DebuggerBase):
    ...
    def _epoch_train(self):
        ...
        for i, (images1, images2, targets, prob, report, id) in enumerate(progress_bar):
            ...
            batch_stop_loss, batch_word_loss, batch_report_loss, batch_loss = 0, 0, 0, 0

            frontal, lateral, avg = self.extractor.forward(images_frontal, images_lateral)
            report_tf, state_c, state_h = self.semantic.forward(avg)
    ...
    def _epoch_val(self):
        ...
        results = {}
        for images1, images2, targets, prob, report, study in progress_bar:
            images_frontal = self._to_var(images1, requires_grad=False)
            images_lateral = self._to_var(images2, requires_grad=False)
    ...
    def _epoch_test(self):
        ...
        print(test_met)
        print("Time:%d" % (end_time - start_time))
        with open(self.model_dir + '/report.csv', 'w', encoding='utf-8-sig', newline='') as f:
            writer = csv.writer(f)
...
if __name__ == '__main__':
    ...
    DATA_Path = './data/CN/Liver'

    parser.add_argument('--imagePth', type=str, default='./dataset/Ultrasonic_datasets/Liver_dataset/Liver_images/',
                        help='path for images')
    parser.add_argument('--vocab_path', type=str, default=DATA_Path + '/vocab.pkl', help='path for vocabulary')
    # parser.add_argument('--data_dir', type=str, default=DATA_Path + '/new_Mammary2.json', help='path for text')
    parser.add_argument('--data_dir', type=str, default=DATA_Path + '/new_Liver2.json', help='path for images')


    parser.add_argument('--report_dim', type=int, default=264)  # Thyroid:394 Mamary:346
