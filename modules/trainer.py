...
class BaseTrainer(object):
    ...
    def _save_checkpoint(self, epoch, test_res, test_gts, save_best=False):
        ...
        if save_best:
            ...
            with open(os.path.join(self.checkpoint_dir, 'report.csv'), 'w', encoding='utf-8-sig', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['Ground Truth', 'Predict'])
                for i in range(len(test_res)):
                    ps = [test_res[i].replace(' ', '')]
                    rs = [test_gts[i].replace(' ', '')]
                    writer.writerow(rs + ps)
