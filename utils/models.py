...
class CoAttention(nn.Module):
    ...
    def forward(self, avg_features, h_sent):
        if self.version == 'v1':
            ...
            return self.v2(avg_features, h_sent)
        elif self.version == 'v3':
            return self.v3(avg_features, h_sent)
        elif self.version == 'v4':
            return self.v4(avg_features, h_sent)
        elif self.version == 'v5':
            return self.v5(avg_features, h_sent)

    # def v1(self, avg_features, semantic_features, h_sent) -> object:
    def v1(self, avg_features, h_sent) -> object:
        """
        only training
        :rtype: object
        """
    ...
    def v2(self, avg_features, h_sent) -> object:
        """
        no bn
        :rtype: object
        """
    ...
    def v3(self, avg_features, h_sent) -> object:
        """
    ...
    def v5(self, avg_features, h_sent):
        W_v = self.W_v(avg_features)
        W_v_h = self.W_v_h(h_sent.squeeze(1))
...
class WordLSTM(nn.Module):
    ...
    def sample(self, features, start_tokens, cuda):
        sampled_ids = torch.from_numpy(np.zeros((np.shape(features)[0], self.n_max))).cuda()
